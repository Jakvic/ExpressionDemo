...
namespace ExpressionDemo
{
    ...
    public delegate object CreateObject(object value);
    ...
    public partial class RefExpr
    {
        ...
        public static object CreateInstance(Type type)
        {
            try
            {
                if (_cacheCtor.TryGetValue(type, out var c)) return c(null);
            }
            catch (Exception ex)
            {
                throw ex;
            ...
            lock (_cacheCtor)
            {
                try
                {
                    ...
                    if (typeInfo.IsClass)
                    {
                        _cacheCtor.Add(type, c = AnalyzeUI.NodeViews.Media.RefExpr.CreateClass<object>(type));
                    }
                    ...
                    {
                        _cacheCtor.Add(type, c = AnalyzeUI.NodeViews.Media.RefExpr.CreateStruct<object>(type));
                    }
        ...
        /// </summary>
        public static MemberInfo SelectMember(IEnumerable<MemberInfo> members,
        ...
        public static string MethodName(MethodInfo method, int skipParameters = 0)
        {
            lock (_cacheName)
            {
                if (_cacheName.TryGetValue(method, out var value)) return value;
